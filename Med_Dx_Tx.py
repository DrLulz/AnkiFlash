...
PDF_DIR = '/Users/drlulz/Downloads/AnkiFlash-master/CURRENT Medical Diagnosis and Treatment Flashcards.pdf'
...
def make_cards(card_front, card_back, SUBSECTION):
    ...
    mname = 'AnkiFlash'
    dname = 'Flashcards' + '::' + 'CURRENT Medical Dx & Tx::{}'.format(SUBSECTION)
...
def bookmarks(PDF):
    ...
    outline_list = PDF_IN.getOutlines()
    main_sections = [item for item in outline_list if not type(item) == list]

    main_section_list = []
    for section in main_sections:
        main_section_list.append(section['/Title'])

    main_section_list = main_section_list[:-1]
    ...
    outlines = [item for item in outlines if not item['/Title'] in main_section_list]
    ...
    for x in bmrks:
        try:
            replaced = re.sub('([0-9])+.\s+', '', x[1])
            TOC.append( (x[0], (next(it)[0] - 1), replaced) )
        except:
            pass
...
def main():
    ...
    start, end, SUBSECTION = (936, 947, u'Health Care-Associated Infections')
    start, end, SUBSECTION = (948, 959, u'Infective Endocarditis')
#    start, end, SUBSECTION = (960, 971, u'Sepsis')
    page_extract(start, end, SUBSECTION)
    shutil.rmtree(TMP_DIR)
    '''
